# Frontend Design Issues & Bad Practices

This document summarizes identified design issues and bad practices in the frontend codebase.

## 1. State Management Issues

### 1.1 Excessive State in BooksPage
**Location:** `src/pages/BooksPage.tsx`

**Problem:**
- `BooksPage` component manages 8+ separate state variables, leading to prop drilling and complex state synchronization
- Multiple related states (`bookToEdit`, `bookToViewPdf`, `bookViewBook`, `selectedBook`) could be consolidated
- State management logic is scattered throughout the component instead of being centralized
...
**Recommendation:**
- Use a state machine (XState) or reducer pattern to manage complex modal/book view states
- Consider Context API or Zustand for global state management
- Extract modal state management into a custom hook

### 1.2 No Global State Management
**Problem:**
- No centralized state management solution (Redux, Zustand, Context API)
- State is duplicated across components
- Books state is managed in `useBooks` hook but also passed down through multiple components

**Recommendation:**
- Implement a lightweight state management solution (Zustand recommended)
- Create contexts for shared state (books, selected book, UI state)

### 1.3 Duplicated State Logic in Custom Hooks
**Location:** `src/hooks/useBooks.ts`, `src/hooks/usePages.ts`, `src/hooks/useSections.ts`

**Problem:**
- All three hooks follow identical patterns with duplicated error handling logic
- Each hook has its own `loading`, `error`, `setError` state
- Similar CRUD operations repeated across hooks

**Recommendation:**
- Create a generic `useResource` hook that handles common patterns
- Extract shared error handling into a utility or context
